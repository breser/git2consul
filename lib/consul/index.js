...
/**
 * For object that requires UrlEncoding
 */
var render_encoded = function(parts, prefix, obj) {

  _.mapObject(obj, function(val, key) {
    if (_.isArray(val)) return;

    if (_.isObject(val)) return render_encoded(parts, prefix + '/' + encodeURIComponent(key), val)
 ...
 * the diff.
 */
var populate_kvs_from_object = function(branch, prefix, obj, existing_kvs, cb, toEncodeKey) {
  var write_kvs = [];
  var delete_kvs = [];
  var candidate_kvs = [];

  toEncodeKey ?  render_encoded(candidate_kvs, prefix, obj) : render_obj(candidate_kvs, prefix, obj);
  ...
  var handle_json_kv_file = function(file_path, kvs, cb) {
      fs.readFile(file_path, {encoding: 'utf8'}, function (err, body) {
        ...
        try {
          ...
          populate_kvs_from_object(branch, create_key_name(branch, file), obj, kvs, cb, true);
        } catch (e) {
    ...
    function extract_and_populate_properties(file_body, common_body, cb) {
      utils.load_properties(file_body, common_body, function (error, obj) {
        if (error) {
          ...
          populate_kvs_from_object(branch, create_key_name(branch, file), obj, kvs, cb, false);
        }
      });
    }
