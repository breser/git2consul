...
var file_modified = function(branch, file, cb) {
  ...
  var handle_properties_kv_file = function(file_path, common_properties_relative_path, file, kvs, cb) {
  ...
  var handle_expanded_keys_with_different_file_types = function(fqf, file, kvs) {
    if (file.endsWith('.json')) {
      handle_json_kv_file(fqf, kvs, cb);
    } else if (file.endsWith('.yaml') || file.endsWith('.yml')) {
      handle_yaml_kv_file(fqf, kvs, cb);
    } else if (file.endsWith('.properties')) {
      handle_properties_kv_file(fqf, branch.common_properties, file, kvs, cb);
    } else {
  ...
  var handle_file = function(fqf, branch, file, cb) {
    ...
    if (branch.expand_keys && branch.expand_keys_diff) {
      ...
      get_kvs(branch, file, function(err, kvs) {
        ...
        handle_expanded_keys_with_different_file_types(fqf, file, kvs);
      });
      ...
      file_deleted(branch, file, function (err) {
        ...
        handle_expanded_keys_with_different_file_types(fqf, file, []);
      });
    } else {
      handle_as_flat_file(fqf, branch, file, cb);
    }
  ...
  if (fs.lstatSync(fqf).isDirectory()) {
    fs.readdir(fqf, function(err, items) {
      for (var i = 0; i < items.length; i++) {
        var item = fqf + '/' + items[i];
        handle_file(item, branch, items[i], cb);
      }
    });
  } else {
    handle_file(fqf, branch, file, cb);
  }
};
