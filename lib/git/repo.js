...
function Repo(repo_config) {
  ...
  Object.defineProperty(this, 'original_branches_count', {value: repo_config.branches.length});
...
Repo.prototype.addBranch = function(branch) {
  ...
  logger.info("branch/tag %s added for repo %s", branch.name, this_obj.name)
};

Repo.prototype.removeBranch = function(branch_name) {
  var this_obj = this;
  this_obj.branch_names.splice(this_obj.branch_names.indexOf(branch_name), 1);
  this_obj.branches[branch_name].deleteBranchFromConsul(function(err) {
    if (err) {
      return logger.error("Deleting branch from consul failed. Reason : %s", err);
    };
  });
  this_obj.branches[branch_name].purgeBranchCache(function(purge_err) {
    if (purge_err) {
      return logger.error("Cache purge failed.  Failing pull. Reason : %s", purge_err);
    };
  });
  delete this_obj.branches[branch_name];
  logger.info("branch/tag %s removed for repo %s", branch_name, this_obj.name)
};

Repo.prototype.updateTagsAsBranches = function(cb) {
  var this_obj = this;
  git_commands.extractTags(this_obj.url, this_obj.repo_config.local_store, function (err, tags) {
    /* istanbul ignore if */
    if (err) return cb(err);

    var tags_without_branches = this_obj.branch_names.slice(this_obj.original_branches_count);

    var new_tags = tags.filter(function (tag) {
      return tags_without_branches.indexOf(tag) < 0
    });

    var deleted_tags = tags_without_branches.filter(function (branch_name) {
      return tags.indexOf(branch_name) < 0
    });

    deleted_tags.forEach(function (tag_name) {
      this_obj.removeBranch(tag_name);
    });
