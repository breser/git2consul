...
exports.init = function (config, repo) {
    ...
    var handle_tags = function (err, tags) {
        /* istanbul ignore if */
        if (err) return logger.error(err);

        var new_tag = tags.filter(function (tag) {
            return repo.branch_names.indexOf(tag) < 0
        });

        new_tag.forEach(function (tag_name) {
            var branch = new Branch(repo.repo_config, tag_name);
            repo.addBranch(branch);
            start_delayed_updater(branch.name);
        });
    };

    var pollForNewTags = function () {
        setInterval(function () {
            git_commands.extractTags(repo.url, repo.repo_config.local_store, handle_tags);
        }, testing ? 10 : /* istanbul ignore next */
        config.interval * 60 * 1000);
    };
