...
function Branch(repo_config, name) {
  ...
  Object.defineProperty(this, 'path_whitelist', {value: repo_config.path_whitelist});
...
/**
 * We chain this callback when listing changes so that we exclude
 * paths that are not whitelisted
 */
var pruneRecords = function(whitelist, cb) {
  return function(err, records) {
    if (!records) return cb(err, records);
    var newRecords = [];

    records.forEach(function(record){
      if(!whitelist || whitelist.some(x => record.path.startsWith(x))) {
...
Branch.prototype.listChangedFiles = function(from_ref, to_ref, cb) {
  git_commands.listChangedFiles(from_ref, to_ref, this.branch_directory, pruneRecords(this.path_whitelist, cb));
};

Branch.prototype.listAllFiles = function(cb) {
  git_commands.listAllFiles(this.branch_directory, pruneRecords(this.path_whitelist, cb));
};

Branch.prototype.listAdditionalPropertyFiles = function(modifiedRecords, cb) {
  git_commands.listAdditionalPropertyFiles(modifiedRecords, this.branch_directory, pruneRecords(this.path_whitelist, cb));
};
