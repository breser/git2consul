...
Branch.prototype.deleteBranchFromConsul = function(cb) {
  logger.warn("Delete branch/tag from consul %s for branch %s in repo %s", this.branch_directory, this.name, this.repo_name);
  consul_broker.deleteBranch(this, function(err) {
    if (err) {
      return cb(err);
    }
  });
