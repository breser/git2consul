...
for (var i=2; i<process.argv.length; ++i) {
  ...
  if(process.argv[i] === '-f' || process.argv[i] === '--seed-file') {
    if(i+1 >= process.argv.length) {
      logger.error("No token provided with --seed-file option");
      process.exit(7);
    }
    global.seeder_file = process.argv[i+1];
  }
}

if (global.seeder_file) {
  config_seeder.set(global.config_key, global.seeder_file, function(err) {
    if (err) {
      logger.error(error)
      process.exit(2)
    };
