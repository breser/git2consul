 ...
 */
module.exports = function(root_directory, git_url, branches, cb) {

  var branch_managers = {};
  
  function BranchManager(branch_directory) {
    this.branch_directory = branch_directory;
  }
  
  BranchManager.prototype.pull = function(cb) {
    console.log('Pulling from %s', this.branch_directory);
    var child = exec('git pull', {cwd: this.branch_directory}, function(err, stdout, stderr) {
  ...
  BranchManager.prototype.currentRef = function(cb) {
    console.log('Checking HEAD ref for %s', this.branch_directory);
    var child = exec('git log -n 1 --pretty=format:"%H"', {cwd: this.branch_directory}, function(err, stdout, stderr) {
      if (stdout) stdout.trim();
  ...
  var createBranchManager = function(branch, cb) {
    ...
    fs.stat(branch_directory, function(err, stat) {
      if (stat) {
        ...
        branch_managers[branch] = new BranchManager(branch_directory + path.sep + 'configuration');
