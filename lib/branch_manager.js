...
exports.createBranchManager = function(repo_config, branch, branch_creation_callback) {
  ...
  var clone = function(bm, cb) {
    console.log('Command is git clone -b %s %s %s', branch, repo_config.url, branch_parent)
    var child = exec('git clone -b ' + branch + ' ' + repo_config.url + ' ' + repo_config.name, {cwd: branch_parent}, function(err, stdout, stderr) {
