...
exports.createBranchManager = function(root_directory, git_url, branch, repo, branch_creation_callback) {

  var branch_parent = root_directory + path.sep + branch;
  var branch_directory = branch_parent + path.sep + repo;
  ...
  var clone = function(bm, cb) {
    console.log('Command is git clone -b %s %s %s', branch, git_url, branch_parent)
    var child = exec('git clone -b ' + branch + ' ' + git_url, {cwd: branch_parent}, function(err, stdout, stderr) {
  ...
  BranchManager.prototype.listChangedFiles = function(from_ref, to_ref, cb) {
    console.log('Listing changed files in repo %s', branch_directory);
    var child = exec('git diff --name-status ' + from_ref + ' ' + to_ref, {cwd: branch_directory}, function(err, stdout, stderr) {
      if (stdout) stdout = stdout.trim();
      if (err) return cb(err);
  ...
  BranchManager.prototype.listAllFiles = function(cb) {
    console.log('Listing all files in repo %s', branch_directory);
    var child = exec('git ls-tree --name-status -r HEAD', {cwd: branch_directory}, function(err, stdout, stderr) {
      if (stdout) stdout = stdout.trim();
      if (err) return cb(err);
  ...
  BranchManager.prototype.handleRefChange = function(target_ref, cb) {
    ...
    if (this.getMostRecentRef() === target_ref) {
      console.log('Branch %s already at most recent version', branch_name);
      return cb(null);
    }
    ...
    if (stat) {
      ...
      return bm.getCurrentRef(function(err, ref) {
        ...
        bm.pull(function(err, msg) {
          if (err) return branch_creation_callback(err);
          console.log("Initialized branch manager: %s", msg);
